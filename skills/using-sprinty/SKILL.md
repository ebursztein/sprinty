...
| `next` | `{ past?=1, future?=3 }` | Compact active work window. No full graph. |
...
| `item_add` | `{ subsprint, title, description, code_locations[], gates[], dependencies?[] }` | Create an atomic item. Title 3-80 chars; description 20-500 chars. Oversized inputs mean create more than one item. |
| `item_get` | `{ id }` | Full item detail. |
| `item_update` | `{ id, note?, dependencies?[] }` | Add an item update and/or dependency ids. |
...
- Dependencies are ids. `item_update({ id, dependencies:[...] })` rejects unknown ids, duplicates, and cycles.

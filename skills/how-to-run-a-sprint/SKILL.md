...
4. Create atomic work with `item_add({ subsprint, title, description, code_locations, gates, dependencies })`. If the work is too large, make more items; do not hide scope in notes.
5. Work against one owning item. Use `item_update({ id, note })` for progress, `item_update({ id, dependencies })` for graph edges, `note_add({ id, text })` only for item-scoped observations, and `artifact_add({ title, path, description, related_items })` for durable files.
...
- Keep responses compact. Prefer `next`, `overview`, `search`, and list/get pairs over full views.
